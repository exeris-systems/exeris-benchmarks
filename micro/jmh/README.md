...
| `ExerisEnterpriseTlsBenchmark` | TLS 1.3 record path (Exeris SPI-native `TlsEngine`, Enterprise Memory-BIO harness) | Comparative variant B7 (SPI-native, in-process BIO path) |
...
Use `B6`/`B7` as integration-level lenses with explicit transport wiring caveats.
`B4` in this repository is the pipeline path (`SslHandler` +
`EmbeddedChannel`) and is not split into B4a/B4b.

## Mandatory comparative evidence (B3/B4/B5/B6/B7)
...
- `AbstractCommunityTlsBenchmark` (B6): FD-owner socket transport. OpenSSL is bound
   ...
   `wrapThroughput`.
- `AbstractEnterpriseTlsBenchmark` (B7): Memory-BIO transport. Handshake and data
   flow use `beginHandshake(out)` plus `bioConnector().inject()` / `drain()` with
   no socket crossing and no syscall.

Benchmark exposure by tier:

- `ExerisCommunityTlsBenchmark` (B6): `wrapThroughput` only.
- `ExerisEnterpriseTlsBenchmark` (B7): `wrapThroughput` and
    `wrapUnwrapRoundTripBioPumpIncluded` and
    `wrapUnwrapRoundTripEngineOnly`.

B7 round-trip scope split:

- `wrapUnwrapRoundTripBioPumpIncluded`: includes Memory-BIO connector choreography
   (`inject`/`drain`) in the measured region.
- `wrapUnwrapRoundTripEngineOnly`: measures direct engine-level
   `wrap -> unwrap(cipher, plain)` without connector pump calls in the measured region.

Do not compare `EngineOnly` and `BioPumpIncluded` values on a single scale without
explicitly labeling the boundary difference.
...
java \
   -Dexeris.tls.enterprise.cryptoProviderClass=com.acme.bench.EnterpriseKernelCryptoProvider \
   -Dexeris.tls.enterprise.memoryProviderClass=com.acme.bench.EnterpriseMemoryProvider \
   -Dexeris.tls.enterprise.certPem=/tmp/enterprise-cert.pem \
   -Dexeris.tls.enterprise.keyPem=/tmp/enterprise-key.pem \
   -jar target/benchmarks.jar ExerisEnterpriseTlsBenchmark
```

> [!WARNING]
> Cross-tier comparison caveat (B6 vs B7): Community `wrapThroughput` and
> Enterprise `wrapThroughput` are not equivalent on a single scale unless the
> transport model is explicitly labeled. B6 includes TLS encrypt + `write(2)`
> kernel crossing (FD-owner/socket), while B7 measures TLS encrypt + in-process
> Memory-BIO write (no syscall). Valid interpretation is intra-tier comparison
> by payload size, or explicitly labeled "including kernel I/O" vs
> "excluding kernel I/O".

## Comparative caveat: B3/B4 vs B6/B7
...
- `B6` and `B7` are Exeris SPI-native `TlsEngine` variants and are only valid when provider wiring is explicitly configured.
- Do not collapse B3/B4 and B6/B7 conclusions without stating implementation and wiring differences, and do not treat B3 vs B4 as a handler-free apples-to-apples comparison.
...
- For Enterprise TLS benchmarks: keep handshake lifecycle deterministic and
