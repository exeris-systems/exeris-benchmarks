...
*By **Arkadiusz Przychocki** · 2026-07-21 (updated 2026-07-30) · categories: performance, benchmarking, jvm*

**Track:** Community · **Benchmark family:** Runtime · **Scenario:** `entity-read-by-id` · **Date:** 2026-07-21 · **Updated:** 2026-07-30 (see *Revision history* below) · **Bench commit:** `60707c4` (triad data); later data `edfcee0` (§7), `152f0a0` (§8), `67c24e0`/`1e262f9` (§4 profiles), `506bea5`/`435af69`/`8b8acfa` (tail), `4798b5a` (controls), `d0fdbd6` (§8 robustness) · **Hardware profile:** `perf-box-amd64`
...
> - **2026-07-30 — the non-heap category breakdown.** §5's non-heap term is decomposed by NMT category from the same 2026-07-29 captures (no re-measurement): **class metadata + code cache + symbol table account for 55–91 % of the non-heap gap**, driven by class count — Exeris loads **1.6× fewer classes than Quarkus-tuned and 2.5–2.6× fewer than Hibernate**. The two mechanisms a reader would expect are **both ruled out**: `GC` is identical across all six cells to 0.01 MiB, `Thread` is flat within noise with 33–35 platform threads on every arm, and `Other` — the `Unsafe`/direct-buffer category — is the one place Exeris is *larger* (6.3–6.4 vs 3.4 / 5.2 MiB). So the footprint win is a **class-loading and JIT-surface** result, not an off-heap or virtual-thread-stack one. The same pass bounds how much NMT can explain at all (55–69 % of non-heap Rss, 63–83 % of its anonymous subset) rather than distributing the remainder.
...
- **Footprint does not flip:** under an equal 2 GiB per-target memory budget, Exeris runs both workloads in **~0.38–0.43 GB RSS** — ~1/2.7 of Quarkus-tuned, ~1/2.9 of Hibernate — on a 256 MB heap vs their 1280 MB. Most of that gap is the heap policy: **at a matched 256 MiB heap the ratio is 1.18× (light) / 1.26× (heavy) vs Quarkus-tuned and 1.51× / 1.80× vs Hibernate** — agent-free medians of n=3 from the 2026-07-29 campaign, which supersedes the agent-laden profiles this report previously quoted (their agent tax is arm-dependent and understated the heavy-contract ratios). Where that remaining difference sits is **no longer open**: it is in **both** heap and non-heap, and **non-heap is the dominant term in three of the four comparisons** (75–79 % of the gap on light, 61 % against Hibernate on heavy; resident heap dominates only heavy vs Quarkus-tuned, at 59 %). Exeris's non-heap is the smallest of the three on *both* contracts, which is the opposite of what §5's own hypothesis predicted (§5). **And that non-heap is class metadata, code cache and symbol table** — 55–91 % of the gap — driven by Exeris loading **1.6× / 2.5–2.6× fewer classes**; `GC` is identical across arms to 0.01 MiB, `Thread` is flat with ~34 platform threads on all three, and `Other` (`Unsafe`/direct buffers) is the one category where Exeris is *larger*. So it is a **class-loading and JIT-surface** win, not an off-heap or Loom-stack one. The durable claim is that Exeris *serves both workloads in a 256 MB heap where Quarkus's policy cannot*, not that its per-object footprint is ~2.7× leaner.
...
### What that non-heap actually is — a class-loading and JIT-surface advantage, not an off-heap one

The split above says *where* the advantage lives. It does not say *what* the non-heap is made of, and the two plausible mechanisms point in opposite directions: an off-heap runtime should show up as *native buffers*, a Loom-first one as *cheaper thread stacks*. A second pass over the same captures reads NMT's own category accounting ([`nonheap-categories.tsv`](../raw/20260729-entity-read-by-id-3way-footprint-decomposition/nonheap-categories.tsv), medians in [`nonheap-categories-medians.tsv`](../raw/20260729-entity-read-by-id-3way-footprint-decomposition/nonheap-categories-medians.tsv); regenerate with `nonheap-breakdown-analyze.sh` from the committed captures — no re-measurement needed). **Neither mechanism is what carries the win.**

**Read the units before the numbers: this table is NMT `committed`, the one above is `smaps` resident.** NMT exposes no per-category residency, so each figure is an *upper bound* on that category's resident size, and the two tables must not be summed. Two limits bound how much of the resident non-heap NMT can explain at all: it tracks only anonymous and malloc-backed memory, so JVM/libc text, the CDS archive and mapped jars are resident but outside its accounting entirely. Measured coverage is **55–69 % of non-heap Rss, or 63–83 % of the anonymous subset** — the anonymous denominator being the only one NMT can fairly be held to (per-run ranges, n=3). The unexplained remainder is real and stated rather than distributed.

| median of n=3, matched 256 MiB heap — NMT **committed**, MiB | Exeris | Quarkus-tuned | Quarkus-Hibernate |
|---|---|---|---|
| **Light** — class metadata (`Class` + `Metaspace` + CDS) | **32.1** | 49.3 | 71.4 |
| — of which `Metaspace` | **15.7** | 31.0 | 49.4 |
| — code cache | **17.7** | 21.9 | 28.3 |
| — symbol table | **3.4** | 6.7 | 11.9 |
| **Heavy** — class metadata | **32.5** | 48.9 | 76.6 |
| — of which `Metaspace` | **16.0** | 30.6 | 53.5 |
| — code cache | **18.8** | 23.4 | 33.1 |
| — symbol table | **3.4** | 6.6 | 13.0 |
| *Loaded classes* (light / heavy) | ***4 673 / 4 723*** | *7 545 / 7 444* | *11 653 / 12 350* |
| `GC`, both contracts | 16.9 | 16.9 | 16.9 |
| `Thread`, both contracts | 1.11–1.19 | 1.39 | 1.07–1.16 |
| `Other` (`Unsafe`/direct buffers), both contracts | 6.3–6.4 | 3.4 | 5.2 |
| File-backed resident (outside NMT), both contracts | 27.2 | 28.5 | 26.1–26.2 |

**Class metadata + code cache + symbol table account for 55–91 % of the non-heap resident gap** — `+24.6` and `+58.4 MiB` on light against Quarkus-tuned and Hibernate (80 % and 62 % of that contract's non-heap gap), `+24.2` and `+67.9 MiB` on heavy (91 % and 55 %). The remaining three categories are controls, and each of them is informative precisely because it is *flat*:

- **`GC` is identical across all six cells** — 16.9 MiB everywhere, medians within **0.01 MiB** and a per-run spread of **0.03 MiB** over all 18 runs. That is the matched-heap manipulation verifying itself from a second instrument: equal heaps buy equal G1 bookkeeping, so none of the gap is GC structures.
- **`Thread` is flat within noise** — 1.01–1.46 MiB across all 18 runs, all three arms overlapping — and all three run **33–35 platform threads** (medians 33–34). There is *no thread-stack footprint advantage here to find*, and a mechanical reason why: virtual-thread stacks are heap-resident `StackChunk` objects, so a Loom-first design **moves** stack memory into the heap term rather than removing it from the footprint. At ~34 platform threads apiece, none of the three arms is in the regime where stack accounting could matter either way.
- **File-backed residency is flat too** (26.1–28.5 MiB, |Δ| ≤ 1.3 MiB), and the CDS archive is byte-identical at 14 008 kB in every cell. The arms differ in *what they load*, not in *which JVM loads it* — an independent check that the JVM images really are matched.
- **`Other` is the one category where Exeris is *larger*:** 6.3–6.4 MiB against 3.4 (Quarkus-tuned) and 5.2 (Hibernate), stable across all 18 runs (6 282–6 586 kB vs 3 481–3 499 and 5 291–5 295). NMT tags `Unsafe.allocateMemory` — the path `ByteBuffer.allocateDirect` takes — as `Other`, so this is *consistent with* Exeris's off-heap buffers, though NMT alone cannot attribute it to a call site. **It is a footprint penalty Exeris carries and still wins with.** Worth stating plainly: off-heap buffers are exactly where a reader would expect the advantage to come from, and this is evidence against that mechanism, not for it.

So the mechanism is **loading less**, not storing it more cheaply, and not off-heap anything. Exeris loads **1.6× fewer classes than Quarkus-tuned and 2.5–2.6× fewer than Hibernate**, and the class-metadata ratio (1.5× / 2.2–2.4×) tracks that count more closely than any per-class efficiency: netting out the fixed 13.7 MiB CDS archive, per-class metadata is **4.0 / 4.8 / 5.1 kB**, so a modest per-class term rides on top of a dominant count term. The code cache follows the same driver — fewer loaded classes, less JIT surface to compile. This is the honest form of a "lean runtime" claim from this dataset: it is a **class-loading and JIT-surface** result, and it is the one part of the footprint story that a framework could close by loading less, rather than a property of where the bytes live.

**Status of this evidence:** exploratory / descriptive, `n=3` interleaved repeats (per-cell max spread 2.5–8.8 %). It carries **no** stage-7 gate artifacts and **no** AB/BA order control — repeating the whole matrix controls time drift, not arm order, and within every repeat the order is community → tuned → Hibernate, the same confound the [sweep report](2026-07-22-entity-read-by-id-memory-cpu-sweep.md) carries. **On this axis that confound is bigger than the ≤ 2 % this report quotes elsewhere, and the right number must be cited:** the [order-counterbalanced control](../raw/20260724-entity-read-by-id-3way-kernel-profile-LIGHT/counterbalanced-cell/) bounds order effects at ≤ 2 % for *throughput and CPU/req*, but it measured **RSS** separately and found Exeris essentially immune (+0.02 %), Quarkus-tuned −3.2 %, and **Hibernate +13.5 %** — which that control attributes to "JIT/metaspace warmup sensitivity", i.e. precisely the term this decomposition isolates as non-heap. Hibernate sits in slot 3 in every repeat here, so its 224 MiB (light) and 267 MiB (heavy) non-heap figures carry a measured ±13 % sensitivity to sequence position. That does not overturn the finding — 13 % of 224 MiB is ~30 MiB against a 91 MiB gap — but it is the bound that belongs on the Hibernate rows, and the Quarkus-tuned comparison (−3.2 %) is the sturdier of the two. **The category breakdown sharpens that caveat into a specific one rather than escaping it:** the control named *metaspace warmup* as the suspected term, and metaspace is exactly what the breakdown measures as dominant — so Hibernate's 49.4 / 53.5 MiB metaspace figures are the ones most exposed to its slot-3 position, and the ±13 % belongs on them first. Cutting the other way, the mechanism the control guessed at is now measured rather than assumed. Per-arm footprint figures are measured facts; read the cross-arm decomposition as directional. It does not license a throughput or ranking claim.
...
6. **A locale-dependent `awk` trap turned the non-heap breakdown's coverage column into a column of zeros.** `mawk` honours the locale's decimal separator, so under a comma-decimal locale (`pl_PL`, `de_DE`, …) `"0.61" + 0` evaluates to **0** — while `jq`, which produced that input, always emits a `.` regardless of locale. Every awk consumer of jq output is exposed. It was caught because a coverage ratio of exactly zero is impossible, not by any check; `nonheap-breakdown-analyze.sh` now pins `LC_ALL=C` with the reason recorded at that line. **Nothing published was affected, and that was verified rather than assumed:** every §5 median above was recomputed from `decomposition-runs.tsv` under `LC_ALL=C` and reproduces to the last digit (233/276/352, 252/317/453 MiB; heap 99/109/123, 104/141/182; residency 0.387/0.427/0.479, 0.406/0.550/0.710) — those figures came from `jq`, and the kB columns are integers, which the bug leaves alone. The same trap one column over in a percentage would have read as a finding.
