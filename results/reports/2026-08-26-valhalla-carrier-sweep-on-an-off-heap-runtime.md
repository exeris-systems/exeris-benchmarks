...
# (CI [+5.93 %, +7.63 %]) while its component -- adaptorCount -- is deterministic (977 in all six
# F runs). Quote the adapter count when one number has to travel alone; it is the one that
...
3. **The measurable, repeatable effect is a cost, not a saving.** The sweep adds **+151
   calling-convention adapters** — 977 in all six F runs, matching the +149 converted carriers —
...
| **adapters** | 825.8 | **977.0** | **+151.2** | +18.306 % | [+17.874, +18.738] | **−0 / +6** |
...
**The adapter count is deterministic.** F reports **977 adapters in all six runs**; E reports 827
in five and 820 in one. +151 adapters against **+149 converted carriers** is a near one-to-one
correspondence — the two-adapter excess is not accounted for here and is not needed for the
argument — and it has a named cause: `InlineTypePassFieldsAsArgs = true` and
...
explain 151; the remaining ~146 are compiled methods. A method-body change in an already-loaded
...
|---|---|---|
| **+151 adapters** | — | **unassailable**: deterministic (977 in 6 of 6 F runs), named mechanism, numerically matched to +149 carriers |
...
| adapters | 809.7 | 975.8 | +166.2 | +20.525 % | [+19.757, +21.294] | −0 / +6 |
...
carriers shaped like the kernel's (`-` means "no valid layout of this kind"):
...
**What `−` means here, because one row reads backwards.** `V8` has no `NULL_FREE_NON_ATOMIC`
layout but does have `NULL_FREE_ATOMIC` — which looks inverted, since non-atomic is the weaker
guarantee. It is not a limit but a *generation* rule, and the null-restricted array probe shows it
directly: asking `ValueClass.newNullRestrictedNonAtomicArray` for an 8-byte two-`String` carrier
produced an array whose layout kind is **`NULL_FREE_ATOMIC_FLAT`**. At 8 bytes the atomic layout is
achievable in a single word, so HotSpot uses it and never needs the non-atomic variant. Read `−` as
"not generated for this class", not as "impossible".

**`NULLABLE_NON_ATOMIC_FLAT` has no size limit** — even a 32-byte payload gets a valid layout.
Every other kind stops at 8 bytes. Three consequences follow, and all three were measured, not
inferred:
...
report promotes findings from exactly one of them. §4's adapter interval is [+17.87 %, +18.74 %]
and survives any multiplicity correction a reader cares to apply; §2.1's two marginal exclusions
clear zero by 0.09 pp and 0.07 pp and would not survive the mildest, which is why §2.1 declines to
