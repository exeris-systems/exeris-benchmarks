...
public class PaymentGatewayClient {
    ...
    private static final String CALLBACK_URL = System.getenv()
            .getOrDefault("EXERIS_PAYMENT_CALLBACK_URL",
                    "http://host.docker.internal:"
                            + System.getenv().getOrDefault("EXERIS_PORT", "9004")
     ...
     * the orderId alone, so a fake decline would corrupt the oracle. The flow parks
    ...
    public void dispatch(String orderId, String sagaId) {
        ...
        http.sendAsync(request, HttpResponse.BodyHandlers.discarding())
                .exceptionally(failure -> {
                    log.warn("payment dispatch failed for saga {} — the flow stays parked", sagaId, failure);
                    return null;
                });
    }
}
