...
import eu.exeris.benchmarks.targets.restateapp.api.Dtos.CartItemView;
...
/**
 * Cart state (Postgres carts/cart_items + graph IN_CART edge) — SQL and graph
 * writes identical to spring-benchmark-app ShopSagaStateService.
 */
public final class ShopSagaStateService {
    ...
    public CartView addToCart(String userId, String productId, int quantity) {
    ...
    public CartView getOrCreateCart(String userId) {
        long uid = Long.parseLong(userId);
        try (Connection conn = dataSource.getConnection()) {
